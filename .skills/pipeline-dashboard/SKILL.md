...
2. **Bug 任务次之** - 编号 10101-19999 的任务优先处理
3. **重构任务** - 编号 20101-29999 按编号顺序执行
4. **测试任务** - 编号 30101-39999 按编号顺序执行
5. **Feature 任务** - 编号 00001-09999 按编号顺序执行
...
|---------------------|---------------------|
| 重构任务 (20101–29999) | 对应的功能实现必须已完成 |
| 测试任务 (30101–39999) | 对应的功能实现必须已完成 |

### 依赖检测逻辑 / Dependency Detection Logic

1. 提取待处理任务的关键词和任务编号
2. 扫描已完成任务，查找对应的依赖任务:
   - Bug 任务 (10101–19999): 检查是否有对应的修复基础
   - 重构任务 (20101–29999): 检查原功能是否已完成
   - 测试任务 (30101–39999): 检查待测功能是否已完成
...
1. 首先按优先级排序: P0 > P1 > P2
2. 按任务类型排序: Bug (10101–19999) > Refactoring (20101–29999) > Testing (30101–39999) > Feature (00001–09999)
3. 同类型内按编号排序
4. 考虑依赖关系，跳过无法执行的任务
...
2. 对每个任务:
   ...
   b) 判断任务类型:
      - Bug: 编号 10101-19999 或文件名包含 _bug_
      - Refactoring: 编号 20101-29999 或文件名包含 _refactor_
      - Testing: 编号 30101-39999 或文件名包含 _test_
      - Feature: 编号 00001-09999
...
| 重构任务 / Refactoring | 20101–29999 | Claude Code |
| 测试任务 / Testing | 30101–39999 | Claude Code |
...
除非任务涉及以下情况，否则统一推荐 Claude Code：

| 优先级 | 任务类型 | 条件 | 执行者 |
|--------|---------|------|--------|
| 1 | 重构任务 | 始终 | Claude Code |
| 2 | 测试任务 | 始终 | Claude Code |
| 3 | Bug 修复 | P0/P1 恶性bug | Claude Code |
| 4 | Bug 修复 | 普通bug | Codex |
| 5 | 功能任务 | 前端设计 | Gemini |
| 6 | 功能任务 | 后端/前端实现 | Codex |
| 7 | 简化任务 | 任何情况 | Claude Code |

### 简化任务类型 / Simple Task Types
...
|------------|----------------------|------------------------------|
| Bug Fix / Security Fix | 10101–19999 | `*_bug_*.md` |
| Refactoring / Tech Debt | 20101–29999 | `*_refactor_*.md` |
| Testing / Quality | 30101–39999 | `*_test_*.md` |
| Feature Development | 00001–09999 | 其他 / others |
...
1. **提取任务编号**: 从待处理文件名提取编号 (如 `10103_bug_xxx.md` -> `10103`，或 `20105_refactor_xxx.md` -> `20105`)
2. **判断类型**: 检查任务类型:
   - Bug 提示词: 编号 10101–19999 或匹配 `*_bug_*.md`
   - 重构提示词: 编号 20101–29999 或匹配 `*_refactor_*.md`
   - 测试提示词: 编号 30101–39999 或匹配 `*_test_*.md`
   - 功能提示词: 编号 00001–09999
