...
```powershell
python test_runner\test_runner_agent.py --command start --test-type quick_smoke
python test_runner\test_runner_agent.py --command report
python test_runner\test_runner_agent.py --command stop --reason "check"
...
for t in ["test_runs","checkpoints","snapshots","anomalies","consistency_checks","workflow_positions","agent_state"]:
    try:
