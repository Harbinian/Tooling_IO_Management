﻿<template>
  <div class="space-y-10 animate-in fade-in duration-500 pb-20">
    ...
    >
      <p class="text-xs font-medium uppercase tracking-[0.24em] text-rose-500">Request failed</p>
      <h2 class="mt-3 text-2xl font-semibold text-slate-900">The order detail could not be loaded</h2>
    ...
    >
      <p class="text-xs font-medium uppercase tracking-[0.24em] text-slate-400">Missing data</p>
      <h2 class="mt-3 text-2xl font-semibold text-slate-900">No detail is available for this order</h2>
      <p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">
        The API returned no order detail for this identifier. Check the route parameter or go back
    ...
    <template v-else>
      <div class="grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <Card class="border-slate-200/80 bg-white/90 shadow-[0_30px_80px_-55px_rgba(15,23,42,0.4)]">
          <CardHeader class="border-b border-slate-100">
            <div class="space-y-1">
              <p class="text-xs font-medium uppercase tracking-[0.24em] text-slate-400">Overview</p>
              <CardTitle class="text-lg text-slate-900">Order information</CardTitle>
            </div>
          </CardHeader>
        ...
        <Card class="border-slate-200/80 bg-white/90 shadow-[0_30px_80px_-55px_rgba(15,23,42,0.4)]">
          <CardHeader class="border-b border-slate-100">
            <div class="space-y-1">
              ...
              <CardTitle class="text-lg text-slate-900">Progress tracker</CardTitle>
            </div>
          </CardHeader>
      ...
      <Card class="border-slate-200/80 bg-white/90 shadow-[0_30px_80px_-55px_rgba(15,23,42,0.4)]">
        <CardHeader class="border-b border-slate-100">
          <div class="space-y-1">
            <p class="text-xs font-medium uppercase tracking-[0.24em] text-slate-400">Items</p>
            <CardTitle class="text-lg text-slate-900">Tool item details</CardTitle>
          </div>
        </CardHeader>
        <CardContent class="pt-0">
          <div v-if="order.items?.length" class="divide-y divide-slate-100">
            ...
            >
              ...
              <div class="space-y-1">
                <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Apply qty</p>
                <p class="text-sm font-medium text-slate-700">{{ item.applyQty ?? '-' }}</p>
              </div>
              <div class="space-y-1">
                <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Approved qty</p>
                <p class="text-sm font-medium text-slate-700">{{ item.approvedQty ?? '-' }}</p>
              </div>
              <div class="space-y-1">
                <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Current location</p>
                <p class="text-sm font-medium text-slate-700">{{ item.currentLocationText || '-' }}</p>
              </div>
              <div class="space-y-1">
                <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Confirmed location</p>
                <p class="text-sm font-medium text-slate-700">{{ item.keeperConfirmLocationText || '-' }}</p>
              </div>
              <div class="space-y-1">
                <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Check result</p>
          ...
          >
            No tool items are attached to this order.
          </div>
        </CardContent>
      </Card>
      ...
      <div class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <Card class="border-slate-200/80 bg-white/90 shadow-[0_30px_80px_-55px_rgba(15,23,42,0.4)]">
          <CardHeader class="border-b border-slate-100">
            <div class="space-y-1">
              <p class="text-xs font-medium uppercase tracking-[0.24em] text-slate-400">Notifications</p>
              <CardTitle class="text-lg text-slate-900">Notification records</CardTitle>
            </div>
          </CardHeader>
          <CardContent class="pt-6">
            <div v-if="order.notificationRecords?.length" class="space-y-3">
              ...
              >
                <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div class="space-y-1">
                    <p class="text-sm font-semibold text-slate-900">
                      {{ record.title || record.notifyType || 'Notification' }}
                    </p>
                ...
                <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                  <span class="rounded-full bg-slate-100 px-2.5 py-1">Type {{ record.notifyType || '-' }}</span>
                  <span class="rounded-full bg-slate-100 px-2.5 py-1">Status {{ record.sendStatus || '-' }}</span>
                </div>
            ...
            >
              Notification records are not available for this order yet.
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div class="grid gap-6 xl:grid-cols-3">
        <div class="relative">
          <NotificationPreview
            type="keeper"
            :content="keeperText"
            empty-text="No keeper request preview"
          />
          <Button
        ...
        <div class="relative">
          <NotificationPreview
            ...
            empty-text="No transport notice preview"
          />
          <Button
        ...
        <NotificationPreview
          type="wechat"
          :content="wechatText"
          empty-text="No copy text available"
        />
      </div>
    </template>
  </div>
...
const overviewFields = computed(() => [
  { label: 'Order type', value: orderTypeLabel(order.value.orderType) },
  { label: 'Initiator', value: order.value.initiatorName || '-' },
  { label: 'Keeper', value: order.value.keeperName || '-' },
  { label: 'Project code', value: order.value.projectCode || '-' },
  { label: 'Usage purpose', value: order.value.usagePurpose || '-' },
  { label: 'Target location', value: order.value.targetLocationText || '-' },
  { label: 'Created', value: formatDateTime(order.value.createdAt) },
  { label: 'Transport type', value: order.value.transportType || '-' },
  { label: 'Tool count', value: order.value.toolCount ?? 0 },
  { label: 'Approved count', value: order.value.approvedCount ?? 0 },
  { label: 'Department', value: order.value.department || '-' },
  { label: 'Remark', value: order.value.remark || '-' }
])

const workflowOrder = ['draft', 'submitted', 'keeper_confirmed', 'transport_notified', 'completed']
  ...
  return [
    {
      ...
      index: '01',
      label: 'Draft',
      description: 'The initiator has prepared the request but not completed the workflow.',
    ...
    },
    {
      ...
      index: '02',
      label: 'Submitted',
      description: 'The order has been sent into the keeper review stage.',
      state: stepState(activeIndex, 1, activeStatus)
    },
    {
      key: 'keeper_confirmed',
      index: '03',
      label: activeStatus === 'partially_confirmed' ? 'Partially confirmed' : 'Keeper confirmed',
      description:
        activeStatus === 'partially_confirmed'
          ? 'Only part of the requested tools have been confirmed so far.'
          : 'Keeper review and quantity confirmation have been completed.',
      state:
        activeStatus === 'partially_confirmed'
          ? 'current'
    ...
    {
      key: 'transport_notified',
      index: '04',
      label: 'Transport / follow-up',
      description: 'Transport handling or final processing is in progress.',
      state:
    ...
    },
    {
      key: 'completed',
      index: '05',
      label: activeStatus === 'rejected' ? 'Rejected' : activeStatus === 'cancelled' ? 'Cancelled' : 'Completed',
      description:
        activeStatus === 'rejected'
          ? 'The order was rejected during workflow review.'
          : activeStatus === 'cancelled'
            ? 'The order was cancelled before completion.'
            : 'The workflow has reached its final completed state.',
      state:
...
const canNotifyTransport = computed(() => {
  return ['keeper_confirmed', 'partially_confirmed', 'transport_notified'].includes(order.value.orderStatus)
})
...
function canGenerateTransportPreview(currentOrder) {
  ...
  return ['keeper_confirmed', 'partially_confirmed', 'transport_notified'].includes(currentOrder.orderStatus)
}
...
async function loadOrder() {
  ...
  try {
    ...
    if (!detailResult.success) {
      errorMessage.value = detailResult.error || 'Failed to load order detail.'
    ...
    transportText.value = ''
    wechatText.value = ''
    errorMessage.value = error?.response?.data?.error || error?.message || 'Failed to load order detail.'
  } finally {
    loading.value = false
  }
}
...
async function cancelCurrentOrder() {
  await ElMessageBox.confirm(`Cancel order ${props.orderNo}?`, 'Cancel order', { type: 'warning' })
  const result = await cancelOrder(props.orderNo, operatorPayload())
  if (result.success) loadOrder()
}

async function finalConfirmCurrentOrder() {
  await ElMessageBox.confirm(`Finalize order ${props.orderNo}?`, 'Final confirm', { type: 'warning' })
...
async function sendKeeperNotification() {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(result.send_status === 'sent' ? 'Keeper notification sent to Feishu' : `Failed: ${result.send_result}`)
      loadOrder()
    } else {
      ElMessage.error(result.error || 'Failed to send notification')
    }
  } catch (e) {
    ElMessage.error('Failed to send keeper notification')
  } finally {
    loading.value = false
  }
}
...
async function sendTransportNotification() {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(result.send_status === 'sent' ? 'Transport notification sent to Feishu' : `Failed: ${result.send_result}`)
      loadOrder()
    } else {
      ElMessage.error(result.error || 'Failed to send notification')
    }
  } catch (e) {
    ElMessage.error('Failed to send transport notification')
  } finally {
    loading.value = false
  }
}
