<template>
  <div class="flex flex-col h-[calc(100vh-8rem)] lg:flex-row gap-8 overflow-hidden animate-in fade-in duration-500 pb-4">
    <Card class="w-full lg:w-[440px] flex flex-col h-full bg-white/50 backdrop-blur-sm border-white/20 shadow-shimmer overflow-hidden">
      <CardHeader class="border-b border-slate-100/50 bg-slate-50/30 py-5 shrink-0">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">待处理队列</p>
    ...
    <Card class="flex-1 flex flex-col h-full bg-white shadow-2xl border-border/50 overflow-hidden">
      <CardHeader class="border-b border-slate-100 py-5 shrink-0">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div>
              <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">处理工作台</p>
          ...
          <div v-if="selectedOrder.orderNo" class="flex gap-3">
            ...
            <Button v-debug-id="DEBUG_IDS.KEEPER.REJECT_BTN" variant="destructive" size="sm" class="font-bold" :disabled="!canReview" @click="rejectCurrentOrder">
              驳回
            </Button>
            <Button v-debug-id="DEBUG_IDS.KEEPER.QUICK_CONFIRM_BTN" variant="default" size="sm" class="font-bold bg-slate-900 text-white shadow-lg" :disabled="!canReview" @click="approveOrder">
              确认通过
            </Button>
          </div>
        </div>
      ...
      <CardContent v-debug-id="DEBUG_IDS.KEEPER.DETAIL_DRAWER" class="flex-1 overflow-y-auto p-0 scrollbar-thin">
        <div v-if="!selectedOrder.orderNo" class="flex flex-col items-center justify-center h-full p-12 text-center">
          ...
          <h3 class="text-lg font-semibold text-slate-900 mb-2">选择一张单据开始处理</h3>
          <p class="text-sm text-slate-500 max-w-sm">
            从左侧列表点击任意待处理单据，即可在这里查看详情并执行保管确认。
          </p>
        </div>
        ...
        <div v-else class="p-6 space-y-8">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            <div class="space-y-2">
              ...
              <Select v-model="confirmForm.transportType">
                <option value="" disabled>请选择运输类型</option>
            ...
            <div class="space-y-2 md:col-span-2">
              ...
              <Textarea v-model="confirmForm.keeperRemark" placeholder="请输入确认时的补充说明" />
            </div>
          </div>
          ...
          <div v-if="canNotify" class="flex items-center justify-between p-4 rounded-xl bg-blue-50/50 border border-blue-100/50">
            <div class="flex items-center gap-3 text-blue-700">
              <Info class="h-5 w-5" />
              <p class="text-sm font-medium">保管确认已完成，可点击右侧按钮发送飞书运输通知。</p>
            </div>
